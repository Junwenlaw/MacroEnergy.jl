...
function run_case(
    ...
    try 
            ...
            try
                ...
                if TDR_flag == 1
                    ...
                    if myTDRsetup["ClusterSubperiodResults"] == 1
                        cpu_subperiod_cases = run_subperiod_cases(case_path, optimizer,optimizer_env, optimizer_attributes, myTDRsetup; num_periods = num_periods, v = false)
                    end
        ...
        if isa(solution_algorithm(case), Benders)
            ...
            # Cluster Subperiod Results: Incorporate Output-based TDR by solving individual subperiod level CEM
            if bd_setup[:BendersCut] in ("group", "group_disaggregated") && bd_setup[:GroupType] == "fixed"
...
"""
    run_stochastic_case(case_path; optimizer, optimizer_env, optimizer_attributes, ...)

High-level entry point for stochastic capacity expansion cases, mirroring `run_case`.

Reads `stochastic_data.json` from `case_path`, loads all scenarios, builds and solves
the stochastic model (monolithic or Benders depending on `case_settings.json`), and
returns `(sc, model, cpu_time)`.
"""
function run_stochastic_case(
    ...
    try
        ...
        # If Benders, create processes for subproblems optimization
        if isa(solution_algorithm(sc), Benders)
            if sc.settings.BendersSettings[:Distributed]
                number_of_subproblems = sum(
                    length(sc_r.system.time_data[:Electricity].subperiods)
        ...
        start_solve = time()
        if isa(solution_algorithm(sc), Benders)
            result = solve_case(sc, opt, Benders(); case_path = case_path)
        else
            result = solve_case(sc, opt)
        end
        cpu_optimization = time() - start_solve

        @info("Writing stochastic outputs")
        (sc_out, model_out, _) = result
        results_dir = write_outputs(case_path, sc_out, model_out)

        cpu_case = time() - start_case
        @info("Stochastic case solved in $(round(cpu_optimization, digits=2)) s " *
              "(total: $(round(cpu_case, digits=2)) s)")
